...
namespace Amellar.Modules.HealthPermit
{
    public class HealthPermitClass
    {
        ...
        public DataTable HealthPermitView(string bin, string sBnsName) //JARS 20170112 ADDED BUSINESS NAME
        {
            ...
            if (result.Execute())
            {
                while (result.Read())
                {
                    string currentY = AppSettingsManager.GetConfigValue("12"); //JARS 20121222
                    if (result.GetString("tax_year") != currentY) //JARS 20151222
                    {
                        OracleResultSet count = new OracleResultSet();
                        count.Query = "select count(*) from emp_names where tax_year = '"+currentY+"'";
                        count.Execute();
                        count.Read();
                        int ID = count.GetInt(0) + 1;
                        string newID = ID.ToString();
                        string newID2 = newID.PadLeft(6, '0');
                        string newExp = "12/31/" + System.DateTime.Today.Year.ToString();
                        string newDate = DateTime.Today.ToString("MM/dd/yyyy"); //JARS 20160104
                        string emp_id = currentY + "-" + newID2;
                        OracleResultSet newResult = new OracleResultSet();
                        #region comments
                        //newResult.Query = "insert into emp_names (bin, tax_year, emp_ln, emp_fn, emp_mi, emp_occupation, emp_age, emp_date_of_birth, emp_gender, emp_nationality, emp_place_of_work, emp_address, emp_tin, emp_ctc_number, emp_ctc_issued_on, emp_ctc_issued_at, emp_status, emp_rh_no, emp_id, EMP_ISSUANCE_DATE, EMP_EXPIRATION_DATE, EMP_XRAY_DATE, EMP_XRAY_RESULT, EMP_HEPAB_DATE, EMP_HEPAB_RESULT, EMP_DRUG_TEST_DATE, EMP_DRUG_TEST_RESULT, EMP_FECALYSIS_DATE, EMP_FECALYSIS_RESULT, EMP_URINALYSIS_DATE, EMP_URINALYSIS_RESULT) ";
                        //newResult.Query += "VALUES ('" + result.GetString("bin") + "','"+ currentY +"', '" + result.GetString("emp_ln") + "', '" + result.GetString("emp_fn") + "', '" + result.GetString("emp_mi") + "', '" + result.GetString("emp_occupation") + "', '" + result.GetInt("emp_age") + "', '" + result.GetString("emp_date_of_birth") + "', '" + result.GetString("emp_gender") + "', '" + result.GetString("emp_nationality") + "', '" + result.GetString("emp_place_of_work") + "'";
                        //newResult.Query += ", '" + result.GetString("emp_address") + "', '" + result.GetString("emp_tin") + "', '" + result.GetString("emp_ctc_number") + "', '" + result.GetString("emp_ctc_issued_on") + "', '" + result.GetString("emp_ctc_issued_at") + "', '" + result.GetString("emp_status") + "', '" + result.GetString("emp_rh_no") + "', '" + emp_id + "', '" + newDate + "','" + newExp + "',";
                        //newResult.Query += "'" + newDate + "', '" + result.GetString("EMP_XRAY_RESULT") + "', '" + newDate + "', '" + result.GetString("EMP_HEPAB_RESULT") + "', '" + newDate + "', '" + result.GetString("EMP_DRUG_TEST_RESULT") + "', '" + newDate + "','" + result.GetString("EMP_FECALYSIS_RESULT") + "','" + newDate + "', '" + result.GetString("EMP_URINALYSIS_RESULT") + "')";
                        #endregion
                        //JARS 20170113 NEW QUERY FOR INSERTING INTO EMP_NAMES
                        newResult.Query = "INSERT INTO EMP_NAMES ";
                        newResult.Query += "(BIN, TAX_YEAR, EMP_LN, EMP_FN, EMP_MI, EMP_OCCUPATION, EMP_AGE, EMP_DATE_OF_BIRTH, EMP_GENDER, EMP_NATIONALITY, ";
                        newResult.Query += "EMP_PLACE_OF_WORK, EMP_ADDRESS, EMP_TIN, EMP_STATUS, EMP_ID, EMP_ISSUANCE_DATE, EMP_EXPIRATION_DATE, EMP_XRAY_DATE, EMP_XRAY_RESULT, EMP_HEPAB_DATE, EMP_HEPAB_RESULT, EMP_DRUG_TEST_DATE, ";
                        newResult.Query += "EMP_DRUG_TEST_RESULT, EMP_FECALYSIS_DATE, EMP_FECALYSIS_RESULT, EMP_URINALYSIS_DATE, EMP_URINALYSIS_RESULT, EMP_RH_NO, BNS_NM, BNS_ADD, TEMP_BIN, EMP_CTC_NUMBER, EMP_CTC_ISSUED_ON, EMP_CTC_ISSUED_AT) ";
                        newResult.Query += "VALUES (";
                        newResult.Query += "'" + result.GetString("bin") + "', '"+ currentY +"', '"+ result.GetString("emp_ln") +"', '"+result.GetString("emp_fn")+"', '"+ result.GetString("emp_mi") +"', '"+ result.GetString("emp_occupation") +"', '"+ result.GetString("emp_age") +"', ";
                        newResult.Query += "'" + result.GetString("emp_date_of_birth") + "', '" + result.GetString("emp_gender") + "', '" + result.GetString("emp_nationality") + "', '" + result.GetString("emp_place_of_work") + "', '" + result.GetString("emp_address") + "', '" + result.GetString("emp_tin") +"', '"+ result.GetString("emp_status") +"', ";
                        newResult.Query += "'" + emp_id + "', '"+ newDate +"', '"+ newExp +"', '"+ newDate +"', '"+ result.GetString("emp_xray_result") +"', '"+ newDate +"', '"+ result.GetString("emp_hepab_result") +"', '"+ newDate +"', '"+ result.GetString("emp_drug_test_result") +"', '"+ newDate +"', '"+ result.GetString("emp_fecalysis_result") +"', ";
                        newResult.Query += "'" + newDate + "', '" + result.GetString("emp_urinalysis_result") + "', '" + result.GetString("emp_rh_no") + "', '', '" + result.GetString("bns_add") +"', '"+ result.GetString("temp_bin") +"', '"+ result.GetString("emp_ctc_number") +"', '"+ result.GetString("emp_ctc_issued_on") +"', '"+ result.GetString("emp_ctc_issued_at") +"')";
                        newResult.Execute();
                        newResult.Commit();
                        newResult.Close();
                        
                    }
                    if(result.GetString("tax_year") == sTaxYear) //JARS 20170103
                    {
                        table.Rows.Add(
