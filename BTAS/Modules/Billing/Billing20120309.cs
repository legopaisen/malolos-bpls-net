...
namespace Amellar.BPLS.Billing
{
    ...
    public class Billing
    {
        ...
        protected virtual void GetMinMaxDueYear(string p_sBIN, out string o_sMinDueYear, out string o_sMaxDueYear)
        {
            ...
            //pSet.Query = "SELECT * FROM pay_hist WHERE bin = :1 AND tax_year =  :2 AND bns_stat <> 'NEW'";
            pSet.Query = "SELECT * FROM pay_hist WHERE bin = :1 AND tax_year =  :2";    // RMC 20120103 modified billing for delinquent years
            pSet.AddParameter(":1", p_sBIN);
            ...
            if (pSet.Execute())
            {
                ...
                else
                {
                    ...
                    pSet.Query = "select max(tax_year) from pay_hist where bin = :1";
                    pSet.AddParameter(":1", p_sBIN);
                    if (pSet.Execute())
                    {
